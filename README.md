...
| agy (Antigravity CLI, Gemini family)                                     | 1.2.2           | vendor release tarball at the pinned build id                | SHA-512 from the vendor manifest    | `/usr/local/bin/agy`                                              | proprietary  |
...
`agy` self-updates on a 15-minute debounce by default; the image sets `AGY_CLI_DISABLE_AUTO_UPDATE=true` so the pinned binary stays the one that was verified at build. Two more worker executors are pinned outside this table because they are package-manager installs, not baked binaries: `@anthropic-ai/claude-code` and `@openai/codex` (exact npm versions in the Dockerfile) and gstack (an exact commit of `garrytan/gstack`, cloned into `/root/.claude/skills/gstack` and built with its own `./setup`).
