...
- **Every bun layer purges bun's download cache in its own RUN.** bun has no equivalent of the `npm cache clean --force` the npm layers run, and `./setup` alone leaves a large download cache behind; `bun pm cache rm` followed by `test ! -d /root/.bun/install/cache` closes it, and a contract test requires both of every bun layer. It has to be the SAME RUN — a later layer cannot delete bytes an earlier one committed. Inside the container this takes `/root/.bun` from 1.4 GB to 339 MB; the image itself moves only ~60 MB, and that gap is unexplained, so measure a real push rather than trusting `docker history`.
- **The bundled browser stays.** gstack ships `GSTACK_SKIP_PLAYWRIGHT=1` (`setup:1088-1089`) and it would save 656 MB, but the browser skills are load-bearing for website work — which is also why the `playwright install-deps chromium` layer exists.
...
- **`bubblewrap` + `socat`** are on the first apt line. They are Claude Code's Linux sandbox dependencies — bubblewrap is "the unprivileged sandboxing tool that enforces filesystem isolation" and socat "the relay used to route network traffic through the sandbox proxy" (`code.claude.com/docs/en/sandboxing`, "Set up Linux and WSL2"). They are not optional: the managed policy sets `sandbox.failIfUnavailable: true`, so a worker session **exits at startup** if either is missing. Debian-versioned rather than exact-pinned, for the same reason the PostgreSQL minor floats — an exact apt pin is a time-bomb when the pool rotates. Contract-tested like `tmux`.
  ...
  Known limitation, not yet measured here: bubblewrap cannot mount a fresh `/proc` in an unprivileged container, and the vendor's fix is `sandbox.enableWeakerNestedSandbox` (`sandboxing.md` → Troubleshooting). The policy sets that key to `true` (see the managed-policy section below: load-bearing, not an optimisation); `sandbox.failIfUnavailable: true` still makes a worker refuse to start, loudly, if the sandbox cannot come up for any other reason. The cost is the vendor's own caveat: the setting "exposes process information to sandboxed commands that a fresh `/proc` mount would hide," which is why `denyRead` carries `/proc`.
...
- **`gstack`** — cloned into `/root/.claude/skills/gstack` and installed with its own `./setup`, per gstack `docs/OPENCLAW.md` "Installation" step 1 and `README.md` "Step 1: Install on your machine". `garrytan/gstack` ships no tags, so the pin is the commit, and the build asserts the resulting `VERSION` file. Only step 1 of that section is image state: the four ClawHub native skills, the AGENTS.md dispatch block and the verification spawn are gateway/volume state. `./setup` needs the Chromium shared libraries the `playwright install-deps` layer already installs — that layer exists for exactly this.
  ...
  **The install path is not free, and a shared `/opt` root does not work.** `setup:65` derives its skills directory as the PARENT of its own location, and `setup:2119,2126` gate the whole Claude install branch on that parent being literally named `skills`. From anywhere else — `/opt/gstack` included — setup takes the else branch at `setup:2166-2168`: *"Not inside a skills/ directory — would symlink the source into `~/.claude/skills/gstack/` and register from there"*, and `setup:2482` pins hook registration to `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/gstack`. So `/opt` buys two paths instead of one. `HOME` is `/root` at build, so the single root-owned path is `/root/.claude/skills/gstack`; a contract test asserts the parent directory name.

  **A worker under a per-client HOME on `/data` does not see that tree.** It gets it through a symlink `~/.claude/skills/gstack -> /root/.claude/skills/gstack` created by the renderer in each worker HOME. **The image seeds no worker HOME.** The tree is left writable rather than `chmod -R a-w`: gstack writes inside its own install (`browse/dist/.version`, `.tmp-bun-bin`), so a read-only tree would break `./setup` and the browser skills.
- **`/etc/claude-code/managed-settings.json`** — the managed Claude Code policy, `COPY`ed from the repo file `claude-code/managed-settings.json`, root-owned `0644`, and parsed by `node` at build so malformed JSON can never reach a worker. `managed-settings.md` ("Place the file on each machine") gives that path for Linux and WSL. Managed settings sit above every other settings level, so nothing in a worker HOME or on `/data` can weaken it. The policy: sandbox on, hard-fail if unavailable, no unsandboxed retry; `denyRead` over `/data/.env`, `/data/agents`, `/data/.openclaw`, `/etc/claude-code`, `/proc`; `credentials.envVars` `deny` for every `GBRAIN_*` variable a worker could otherwise read out of its environment (**names only — never a value; the file ships in the image**); `enableWeakerNestedSandbox` (see below); reads outside the working directories blocked; permission rules and the MCP allowlist managed-only; claude.ai connectors off; one allowed MCP endpoint, `http://127.0.0.1:3131/*`. `tests/unit/managed-settings.test.mjs` asserts each of those and rejects any key outside the documented set — a typo'd key is silently ignored by Claude Code, so the policy would read as enforced while doing nothing.

  **`sandbox.enableWeakerNestedSandbox: true` is load-bearing here, not an optimisation.** sandboxing.md → Troubleshooting: *"in an unprivileged container, bubblewrap can't mount a fresh `/proc` filesystem, so sandboxed commands fail with a `bwrap` error such as `Can't mount proc on /newroot/proc: Operation not permitted`. Set `enableWeakerNestedSandbox` to `true` so the inner sandbox bind-mounts the container's existing `/proc` instead."* The openclaw container is unprivileged, and with `failIfUnavailable: true` the sandbox failing to start means **every worker session exits at startup**. The cost is stated by the vendor and accepted: *"which exposes process information that a fresh mount would hide. This reduces security; use it only when the outer container already provides the isolation you need"* (`settings-reference#sandbox-enableweakernestedsandbox`). The compensating control is that `/proc` stays in `denyRead`, so the filesystem layer still refuses it — whether that in turn breaks `node` under a bind-mounted `/proc` is a question for the local probe, not for this file.

  **`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` is deliberately absent.** It would *"strip credentials from all subprocesses regardless of sandboxing"* (sandboxing.md, "Protect credentials"), which is broader than `credentials.envVars`, but it has **no entry in `settings-reference.md`** — it is an environment variable (`/docs/en/env-vars`), not a settings key, so this file cannot deliver it. The place it could be set is the acpx alias `env -i` allowlist in the gateway config, and it carries a consequence to measure first: it *"turns auto-allow off"* (`settings-reference#sandbox-autoallowbashifsandboxed`), which would make every sandboxed Bash command prompt — fatal for an unattended oneshot worker.
